...
func (uq *UserQuery) loadBudgets(ctx context.Context, query *BudgetQuery, nodes []*User, init func(*User), assign func(*User, *Budget)) error {
	...
	for _, n := range neighbors {
		fk := n.user_budgets
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_budgets" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_budgets" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
