...
func (aq *AccountQuery) QueryBudget() *BudgetQuery {
	query := (&BudgetClient{config: aq.config}).Query()
...
func (aq *AccountQuery) Clone() *AccountQuery {
	...
	return &AccountQuery{
		...
		withBudget:       aq.withBudget.Clone(),
...
func (aq *AccountQuery) WithBudget(opts ...func(*BudgetQuery)) *AccountQuery {
	query := (&BudgetClient{config: aq.config}).Query()
...
func (aq *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := aq.withBudget; query != nil {
		if err := aq.loadBudget(ctx, query, nodes, nil,
			func(n *Account, e *Budget) { n.Edges.Budget = e }); err != nil {
			return nil, err
		}
	}
...
func (aq *AccountQuery) loadBudget(ctx context.Context, query *BudgetQuery, nodes []*Account, init func(*Account), assign func(*Account, *Budget)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Account)
	for i := range nodes {
		if nodes[i].budget_accounts == nil {
			continue
		}
		fk := *nodes[i].budget_accounts
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "budget_accounts" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
