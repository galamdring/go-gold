...
import (
	...
	"github.com/galamdring/go-gold/ent/migrate"
...
// QueryBudget queries the budget edge of a Account.
func (c *AccountClient) QueryBudget(a *Account) *BudgetQuery {
	...
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, account.BudgetTable, account.BudgetColumn),
		)
...
// QueryAccounts queries the accounts edge of a Budget.
func (c *BudgetClient) QueryAccounts(b *Budget) *AccountQuery {
...
// QueryTransactions queries the transactions edge of a Budget.
func (c *BudgetClient) QueryTransactions(b *Budget) *TransactionQuery {
	query := (&TransactionClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := b.ID
		step := sqlgraph.NewStep(
...
// QueryBudget queries the budget edge of a Transaction.
func (c *TransactionClient) QueryBudget(t *Transaction) *BudgetQuery {
	query := (&BudgetClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := t.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(transaction.Table, transaction.FieldID, id),
			sqlgraph.To(budget.Table, budget.FieldID),
			sqlgraph.Edge(sqlgraph.M2O, true, transaction.BudgetTable, transaction.BudgetColumn),
		)
		fromV = sqlgraph.Neighbors(t.driver.Dialect(), step)
		return fromV, nil
	}
...
type (
	hooks struct {
		Account, Budget, Category, Transaction, User []ent.Hook
	}
	inters struct {
		Account, Budget, Category, Transaction, User []ent.Interceptor
	}
)
