...
func (tq *TransactionQuery) QueryBudget() *BudgetQuery {
	query := (&BudgetClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TransactionQuery) Clone() *TransactionQuery {
	...
	return &TransactionQuery{
		...
		withBudget:  tq.withBudget.Clone(),
...
func (tq *TransactionQuery) WithBudget(opts ...func(*BudgetQuery)) *TransactionQuery {
	query := (&BudgetClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TransactionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Transaction, error) {
	...
	if tq.withAccount != nil || tq.withBudget != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := tq.withBudget; query != nil {
		if err := tq.loadBudget(ctx, query, nodes, nil,
			func(n *Transaction, e *Budget) { n.Edges.Budget = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TransactionQuery) loadBudget(ctx context.Context, query *BudgetQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *Budget)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Transaction)
	for i := range nodes {
		if nodes[i].budget_transactions == nil {
			continue
		}
		fk := *nodes[i].budget_transactions
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "budget_transactions" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
